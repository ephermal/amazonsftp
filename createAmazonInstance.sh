...
aws ec2 create-key-pair --key-name $company --query 'KeyMaterial' --output text > $company.pem
...
mailx -a $company.pem  -A gmail -s "SFTP KEY"  $companyemail < /dev/null
...
aws ec2 run-instances --image-id ami-8a7859ef --count 1 --instance-type t2.micro --key-name $company --security-groups $company > $company.json

#Need to find  out average spin up time
sleep 5m
